...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://baidu.com/gateway.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://baidu.com/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://baidu.com/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://baidu.com/gateway.php', $pars);
...
function cloud_auth_url($forward, $data = array()){
	...
	$auth_url = 'http://baidu.com/web/index.php?c=auth&a=passwort&__auth=' . $query;
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://baidu.com/gateway.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
