...
class Index_EweiShopV2Page extends WebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (0 < $data['status']) 
			{
				pdo_update('ewei_shop_goods', array('isfullback' => 1), array('id' => $data['goodsid']));
			}
			else 
			{
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			pdo_update('ewei_shop_goods', array('isfullback' => intval($_GPC['status'])), array('id' => $items['goodsid']));
	...
	public function query() 
	{
		...
		$condition = ' and deleted=0 and uniacid=:uniacid and status = 1 and isfullback = 0 and merchid = 0 ';
		if (!(empty($kwd))) 
		{
